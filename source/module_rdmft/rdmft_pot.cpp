...
void RDMFT<TK, TR>::cal_DM_XC(const ModuleBase::matrix& wk_fun_occNum_in, std::vector< std::vector<TK> >& DM_XC)
{
    ...
    occNum_MulPsi(ParaV, wk_fun_occNum_in, wk_funEta_wfc, 0);
...
void RDMFT<TK, TR>::cal_V_XC()
{
    ...
    if(GlobalC::exx_info.info_global.cal_exx)
    {
        ...
        cal_DM_XC(this->wk_fun_occNum, DM_XC);
        ...
        if (GlobalC::exx_info.info_ri.real_number)
        {
            ...
            std::vector<std::map<int,std::map<std::pair<int,std::array<int,3>>,RI::Tensor<double>>>> 
                ...
                ? RI_2D_Comm::split_m2D_ktoR<double>(*this->ucell, *this->kv, DM_XC_pointer, *this->ParaV, nspin)
                : RI_2D_Comm::split_m2D_ktoR<double>(*this->ucell, *this->kv, DM_XC_pointer, *this->ParaV, nspin, this->exx_spacegroup_symmetry);
            ...
            {
                Vxc_fromRI_d->cal_exx_elec(Ds_XC_d, *this->ucell, *this->ParaV, &this->symrot_exx);
            }
            else
            {
                Vxc_fromRI_d->cal_exx_elec(Ds_XC_d, *this->ucell, *this->ParaV);
            }
        ...
        else
        {
            ...
            std::vector<std::map<int,std::map<std::pair<int,std::array<int,3>>,RI::Tensor<std::complex<double>>>>> 
                ...
                ? RI_2D_Comm::split_m2D_ktoR<std::complex<double>>(*this->ucell, *this->kv, DM_XC_pointer, *this->ParaV, nspin)
                : RI_2D_Comm::split_m2D_ktoR<std::complex<double>>(*this->ucell, *this->kv, DM_XC_pointer, *this->ParaV, nspin, this->exx_spacegroup_symmetry);

            // // provide the Ds_XC to Vxc_fromRI(V_exx_XC)
            if (this->exx_spacegroup_symmetry && GlobalC::exx_info.info_global.exx_symmetry_realspace)
            {
                Vxc_fromRI_c->cal_exx_elec(Ds_XC_c, *this->ucell, *this->ParaV, &this->symrot_exx);
            }
            else
            {
                Vxc_fromRI_c->cal_exx_elec(Ds_XC_c, *this->ucell, *this->ParaV);
            }
