...
template<typename TX>
void BFGS_ONs<TX>::init(const int dim_in, const int nk_total_in)
{
...
void BFGS_ONs<TX>::get_pk(const std::vector<TX>& dE_dx_new, const std::vector<TX>& x_new, std::vector<TX>& pk, const bool new_landscape, const std::vector<TX>* d2E_dx2)
{
    ...
    else
    {
        ...
        rdmft::Tgemm_lapack(this->diff_grad.data(), this->diff_x.data(), &this->rho, 1, 1, this->dim, 'T', 'N');
        ...
        rdmft::Tgemm_lapack( this->diff_x.data(), this->diff_grad.data(), this->rho_diffX_diffGrad.data(), this->dim, this->dim, 1, 'N', 'T', -(this->rho) );
        for(int i=0; i<this->dim; ++i) { this->rho_diffX_diffGrad[i*this->dim + i] += 1.0; }

        // cal rho * diffX * diffX^T
        rdmft::Tgemm_lapack( this->diff_x.data(), this->diff_x.data(), this->rho_diffX_diffX_T.data(), this->dim, this->dim, 1, 'N', 'T', this->rho );
        ...
        rdmft::Tgemm_lapack( H_tmp.data(), this->rho_diffX_diffGrad.data(), this->Hk.data(), this->dim, this->dim, this->dim, 'N', 'T' );
        // rdmft::Tgemm_lapack( this->rho_diffX_diffGrad.data(), this->Hk.data(), this->Hk.data(), this->dim, this->dim, this->dim );
        // rdmft::Tgemm_lapack( this->Hk.data(), this->rho_diffX_diffGrad.data(), this->Hk.data(), this->dim, this->dim, this->dim, 'N', 'T' );
    ...
    else
    {
        ...
        rdmft::Tgemm_lapack( this->Hk.data(), dE_dx_precond.data(), this->search_direction.data(), this->dim, 1, this->dim, 'N', 'N', -1.0 );
    }
