                            ...
                            bool if_iter_diag)
{
    ...
    this->process_file = temp_str.str();
#endif
...
void RDMFT<TK, TR>::cal_Hk_Hpsi()
{
    ...
    for(int ik=0; ik<nk_total; ++ik)
    {
        ...
        if( !only_exx_type )
        {
            ...
            cal_bra_op_ket( ParaV, para_Eij, wfc(ik, 0, 0), H_wfc_dft_XC(ik, 0, 0), Eij_dft_XC );
            _diagonal_in_serial( para_Eij, Eij_dft_XC, &(wfcHwfc_dft_XC(ik, 0)) );
...
double RDMFT<TK, TR>::cal_Energy(const int cal_type)
{
    ...
    if( cal_type == 1 )
    {
        ...
        // if( PARAM.inp.dft_opti && occ_num_opti == false ) { this->Etotal += E_entropy; }
